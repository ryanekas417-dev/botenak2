...
# FIX BAGIAN INI (Parameter chat_id ditambahkan)
async def check_membership(user_id: int):
    ...
    for target in targets:
        try:
            ...
            if m.status in ("left", "kicked", "left"): return False
        except: continue
    return True
...
# ================= START HANDLER (FIXED STRICT FSUB) =================
...
async def start_handler(m: Message):
    ...
    if not await check_membership(m.from_user.id):
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="🔄 SAYA SUDAH JOIN", url=retry_url)]
        ])
        return await m.answer("⚠️ **AKSES DIKUNCI**\n\nSilahkan bergabung dengan channel kami terlebih dahulu untuk menggunakan bot atau melihat konten.", reply_markup=kb)

    # JIKA SUDAH JOIN & ADA KODE KONTEN
    if code:
        async with aiosqlite.connect(DB_NAME) as db:
...
async def admin_upload(m: Message, state: FSMContext):
    ...
    m_cap = m.caption if m.caption else "" # Ambil caption langsung
    
    await state.update_data(temp_fid=fid, temp_type=mtype, temp_cap=m_cap)
    await state.set_state(PostMedia.waiting_for_post_title)
    await m.reply("📝 **JUDUL POSTINGAN (Channel):**", reply_markup=await get_titles_kb())
...
async def select_title_handler(c: CallbackQuery, state: FSMContext):
    ...
    await finalize_post(c.message, state, title)
...
async def process_save_title(m: Message, state: FSMContext):
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        await db.commit()
    await finalize_post(m, state, m.text)

async def finalize_post(msg, state, p_title):
    data = await state.get_data()
    code = uuid.uuid4().hex[:15]
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("INSERT INTO media VALUES (?,?,?,?)", (code, data['temp_fid'], data['temp_type'], data['temp_cap']))
        await db.commit()
    ...
    await msg.answer(f"✅ **POSTED!**\nLink: `{link}`")
    await state.clear()

# ================= ADMIN HANDLERS (TIDAK BERUBAH) =================
...
async def settings_cb(c: CallbackQuery):
    ...
    ch = await get_config("channel_post", "Belum diset")
    fs = await get_config("fsub_channels", "Belum diset")
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
# ================= MEMBER INTERACTION (TIDAK BERUBAH) =================
@dp.callback_query(F.data == "menu_ask")
async def ask_btn(c: CallbackQuery, state: FSMContext):
    await c.message.answer("Kirim pertanyaanmu:"); await state.set_state(MemberStates.waiting_for_ask)
...
async def donasi_btn(c: CallbackQuery, state: FSMContext):
    await c.message.answer("Kirim donasi:"); await state.set_state(MemberStates.waiting_for_donation)

@dp.message(MemberStates.waiting_for_donation)
async def process_donation(m: Message, state: FSMContext):
    await m.copy_to(OWNER_ID, caption=f"🎁 **DONASI** dari `{m.from_user.id}`")
    await m.reply("✅ Terkirim."); await state.clear()

@dp.callback_query(F.data == "menu_vip")
async def order_vip(c: CallbackQuery, state: FSMContext):
    qris = await get_config("qris_file_id")
    if not qris: return await c.answer("QRIS kosong.")
    await bot.send_photo(c.message.chat.id, qris, caption="Kirim SS bukti bayar:"); await state.set_state(MemberStates.waiting_for_vip_ss)
...
async def process_vip_ss(m: Message, state: FSMContext):
    ...
    await m.copy_to(OWNER_ID, caption=f"💎 **SS VIP** dari `{m.from_user.id}`", reply_markup=kb)
    await m.reply("✅ Menunggu konfirmasi."); await state.clear()

@dp.callback_query(F.data == "vip_preview")
async def preview_vip(c: CallbackQuery):
    prev = await get_config("preview_msg_id")
    if not prev: return await c.answer("Preview kosong.")
...
async def add_admin_cb(c: CallbackQuery, state: FSMContext):
    await c.message.answer("Kirim ID Admin:"); await state.set_state(AdminStates.waiting_for_new_admin)
...
async def set_post_cb(c: CallbackQuery, state: FSMContext):
    await c.message.answer("User Channel:"); await state.set_state(AdminStates.waiting_for_channel_post)
...
async def process_addlist(m: Message, state: FSMContext):
    await set_config("addlist_link", m.text.strip()); await m.reply("✅ Set."); await state.clear()
...
async def save_cover(m: Message, state: FSMContext):
    await set_config("cover_file_id", m.photo[-1].file_id); await m.reply("✅ Set."); await state.clear()
...
async def save_qris(m: Message, state: FSMContext):
    await set_config("qris_file_id", m.photo[-1].file_id); await m.reply("✅ Set."); await state.clear()
...
async def save_preview(m: Message, state: FSMContext):
    await set_config("preview_msg_id", str(m.message_id)); await m.reply("✅ Set."); await state.clear()
...
async def broadcast_cb(c: CallbackQuery, state: FSMContext):
    await c.message.answer("Pesan BC:"); await state.set_state(AdminStates.waiting_for_broadcast)
